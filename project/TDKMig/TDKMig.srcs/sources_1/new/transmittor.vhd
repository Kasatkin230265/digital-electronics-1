...
entity transmittor is
    port(
        clk     : in  std_logic;
        reset   : in  std_logic;
        char_i  : std_logic_vector(6 - 1 downto 0);
...
--  p_clocking : process(length_i, reset)
 
--    begin
--        if (reset = '1') then   -- Synchronous reset
--                s_cnt_local <= 0; -- Clear all bits

--            elsif (s_en = '1') then -- Test if counter is enabled
--                if rising_edge(clk) then
--                    if (s_cnt_local < length_i) then
--                        s_light <= temp(s_cnt_local);
